...
읽는 법 — 두 번 크게 틀린 뒤에 얻은 규칙이다:
  ...
  * 단일 인덱스로 안 나오는 표정이 있다. 눈 뜨기가 그랬다 — 11 은 혼자 주면 얼굴을
    ...
    이 조합은 **손그림에서도 된다**. 사진(joyvasa_005)에서만 확인했던 걸 대상 일러스트
    (assets_characters/u_524599, 대형 만화 눈)로 재측정해 pipeline.py 의 EYE_WIDE 로
    넣었다 — 스윕 수치·안전 상한·깜빡임 간섭은 그 상수 옆 주석에 있다. 요지만:
    전역 이동 0(11·15 의 함정이 이 비율에서는 안 걸린다), 0.030 까지 깨끗, 0.060 붕괴.

  * **눈 대역은 `bands()` 로 판정하지 마라.** 이 그림은 홍채가 눈구멍을 거의 채워서
    눈꺼풀이 움직여도 눈 대역 변화 픽셀 수가 잘 안 늘고, 대신 **드러난 흰자 면적**이
    선형으로 는다. 눈 박스 안에서 밝고(>190) 붉은기 없는(R-B<22) 픽셀을 세는 쪽이
    민감하다. 세로 높이는 속눈썹이 고정이라 지표가 안 된다(0.005~0.06 내내 64px 고정).
"""
import argparse
import sys
