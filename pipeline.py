...
#
# 실측(u_524599, 발화 134프레임, blink_interval=4.0). 512 프레임의 눈 박스
# (165,175)-(350,232) 에서 어두운 픽셀(<90 = 홍채+속눈썹)을 세고, 깜빡임 최저 프레임을
# 중앙값(열린 상태)과 비교했다:
#     중립     중앙값 2739 → 최저  438  (-84%)  — 프레임을 보면 완전히 감긴다
#     surprise 중앙값 2756 → 최저 1414  (-49%)  — 좁아지지만 홍채가 계속 보인다
# 즉 **깜빡임이 사라지진 않고 얕아진다.** 수치만으로는 "감기긴 한다" 와 "덜 감긴다" 가
# 구분이 안 돼서 두 최저 프레임을 눈으로 확인했다 — 지표를 바꾸면 절대값은 달라지니
# (박스·문턱에 민감하다) 고칠 땐 위 박스와 문턱을 그대로 쓰고 비교하라.
#
# 두 스케줄을 서로 알게 만들면 없앨 수 있는데 새 결합이라 여기선 안 했다 — 고칠 거면
# blink 프레임에서 눈 축만 0 으로 접는 경로가 된다.
